...
**Зашифрованный бэкап базы (поверх Этапа 4)** – **сделан и device-verified** на Samsung
Galaxy A52. Зачем: ключ основной БД лежит в Keystore и теряется при переустановке/сбросе,
т.е. без бэкапа данные невосстановимы. Решения пользователя: в копию идут **только записи**
(предмет/место/даты; синонимы/история не входят); импорт **заменяет всё**; **отдельный
текстовый пароль** (независим от Keystore/PIN); запуск **вручную** из Настроек.
- Формат файла `.rmdy` (самодостаточный, без внешних либ): `MAGIC "RMDY" | version | salt(16)
  | iv(12) | AES-256-GCM(payload)`. Payload – бинарный (`BackupCodec`): version, count, затем
  для каждой записи name/location (writeUTF) + createdAt/updatedAt.
- Крипта (`security/BackupCrypto`): ключ = **PBKDF2-HMAC-SHA256(UTF-8(пароль), salt, 210000,
  256 бит)**. PBKDF2 реализован сам поверх `javax Mac`, а пароль кодируется в UTF-8 ЯВНО –
  чтобы деривация была детерминированной и не-ASCII (кириллический) пароль давал один ключ
  на любом устройстве (иначе восстановление на новом телефоне могло бы не сойтись из-за
  провайдеро-зависимой кодировки char[] в PBEKeySpec). Проверено против опубликованных
  тест-векторов PBKDF2-HMAC-SHA256. Неверный пароль/битый файл → GCM-тег не сходится →
  WrongPasswordException; чужой файл → BadBackupFileException. Пароль (CharArray) зануляется.
- Репозиторий: `exportRecords()` и `replaceAllRecords()` (в транзакции: явная очистка
  synonyms/history/items → вставка с пересчётом name_norm, `insertIgnore` от коллизий).
- UI (`SettingsActivity`, под гейтом): карточка «Резервная копия» – экспорт (диалог пароля
  ввод+повтор+«показать»+предупреждение → SAF `CreateDocument` `gde-lezhit-YYYYMMDD.rmdy`) и
  импорт (SAF `OpenDocument` → пароль → расшифровка/декод → подтверждение замены с числом
  записей → replace). SAF = без storage-разрешений; **INTERNET по-прежнему нет**.
- Тесты: +18 JVM (BackupCryptoTest 11: roundtrip, неверный/битый/усечённый, версия, PBKDF2-
  векторы, кириллический пароль, интеграция codec+crypto; BackupCodecTest 7). Всего 64 теста.
- Adversarial-ревью (2 агента): исправлены детерминированная деривация не-ASCII пароля
  (главное), тихий no-op при потере пароля (process death) → тост, импорт пустой копии → блок
  вместо стирания. Отклонены (обоснованно): лимит writeUTF (недостижим/перехвачен), рост
  итераций до 600k (заметная задержка на A52 своей PBKDF2-петлёй).
- **Device-verified** на A52: экспорт→файл `.rmdy` (магия RMDY, без плейнтекста, размер по
  формату) → **удаление приложения (потеря Keystore-ключа)** → чистая переустановка (замок
  вкл по умолчанию, задать код) → импорт с верным паролем → **2 записи вернулись с исходными
  датами**; неверный пароль → ошибка, данные целы; шифрование основной БД не регрессировало.
...
### Доработки после ревизии (device-verified на A52, обе темы)

- **Нейтральная палитра** – убран жёлтый (медовый) акцент: вторичные кнопки нейтрально-серые,
  фон нейтральный тёплый; единственный цветной акцент – терракота (`ui/theme/Color.kt`).
- **Фикс цвета текста** – `RemindyTheme` оборачивает контент в `Surface`, задавая `onBackground`
  как `LocalContentColor`. Иначе `Text` без явного цвета = чёрный → в тёмной теме не виден на
  экранах без Scaffold (замок/подтверждение/поиск).
- **Масштаб интерфейса** (доступность, слабое зрение): `ui/UiScale.kt` – множитель `LocalDensity`
  зумит весь UI (Обычный/Крупный/Очень крупный). Выбор в настройках, применяется мгновенно и
  персистентно (наблюдаемое состояние процесса + SharedPreferences). Клавиатура кода стала
  адаптивной по ширине (`BoxWithConstraints`) – не вылезает при 1.5×. Грузится в onCreate каждой
  Activity до setContent.
- **Фикс ложного тоста «Не удалось открыть данные»**: `catch (Exception)` вокруг suspend-вызовов
  ловил `CancellationException` при выходе с экрана (отмена `lifecycleScope`) и показывал ошибку
  БД. Теперь `CancellationException` пробрасывается. Поправлено в List/Search/Confirmation/Unlock.
