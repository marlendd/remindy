...
**Краткий онбординг (поверх Этапа 4)** – **сделан и device-verified** на A52. Отдельный
`OnboardingActivity` (Compose): заголовок «Где лежит», вступление, 3 шага с иконками (Сказать → назвать
вещь и место; Найти → спросить голосом; Список → всё записанное), строка «всё на телефоне,
без интернета», крупная кнопка «Понятно». Показывается автоматически на первом запуске (флаг
`seen` в SharedPreferences, `ui/OnboardingPrefs`; после чистой установки покажется снова) и по кнопке
«Как пользоваться» в Настройках. На первом запуске запрос микрофона отложен до закрытия
онбординга (иначе системный диалог перекрыл бы подсказку). Собрано, 64 теста + lint (0 ошибок)
зелёные. Device-verified: первый запуск показывает → «Понятно» → главный + запрос микрофона;
перезапуск не показывает; из Настроек открывается снова.
...
### Фиксы полного ревью проекта (код готов, device-тест предстоит)

Полное ревью всех исходников (2026-07-19) → исправлено:

- **VoskModelHolder**: загрузка модели в `withContext(NonCancellable)` – уход с экрана
  во время распаковки больше не выбрасывает готовую Model (~50 МБ нативной памяти
  утекало без close) и не запускает вторую распаковку параллельно первой.
- **Поиск**: `SearchTarget.locationNorm` – слово запроса, не найденное в имени,
  добирается местом («паспорт в ящике» теперь находит паспорт); но без единого
  совпадения по имени/алиасу score = 0 (запрос «ящик» не вываливает всё из ящика).
  `SearchActivity.onResume` повторяет последний поиск – после правки/удаления
  результаты не устаревают. «Запомнил» показывается ПОСЛЕ реальной записи синонима.
- **ConfirmationActivity**: правка уже удалённой записи (устаревший тап) → тост
  «Эта запись уже удалена» + закрытие, а не ложное «Сохранено»/пустая форма.
- **AppPin**: конструктор `AppPin(File, clock)` (Context-обёртка осталась) → логика
  тестируется на JVM (`AppPinTest`, 8 тестов: лок, эскалация, персистентность, битые
  файлы, кламп часов); `lockRemainingMs()`; перевод часов назад капится MAX_LOCK_MS.
- **UnlockActivity**: активный лок показывается сразу при входе (не после набора
  кода впустую); CharArray-копии кода и буфер ввода затираются (best-effort).
- **MainActivity**: `protectFromRecents` (живой текст диктовки не попадает в снимок
  «недавних»); диалоги об отказе микрофона – только по тапу «Сказать» (авто-запрос в
  onCreate при «навсегда отклонён» больше не показывает диалог при каждом запуске).
- **DatabaseKey**: raw-key собирается напрямую в байты, без промежуточного String
  (hex полного ключа не виснет в куче). `atomicWrite` вынесен в общий
  `security/AtomicWrite.kt`. Манифест: `configChanges` у ListActivity как у всех.
- **Сборка**: SHA-256-пин zip модели Vosk в `DownloadVoskModelTask` (посчитан с
  локального архива device-verified модели; целый кэш не перекачивается).

## Релиз v1.0.0 (иконка + подпись + release-buildType) – собран, статически проверен

Первый релизный APK. Дистрибуция – **ручная установка APK** (не Play), R8 **выключен** для v1,
ключ подписи **новый** (решения пользователя).

- **Иконка приложения** (`открытая коробка`, до этого `android:icon` не было – дефолтная
  робо-иконка; вариант «метка-пин» отвергнут – похож на карты): adaptive-icon, силуэт открытой
  коробки (тело + две створки с notch), отцентрован (54,54), терракота `#B0552F` на песке
  `#EADDD1`. Файлы:
  `res/drawable/ic_launcher_foreground.xml` + `ic_launcher_monochrome.xml` (темы Android 13+),
  `res/values/ic_launcher_background.xml` (цвет фона), `res/mipmap-anydpi-v26/ic_launcher.xml` +
  `ic_launcher_round.xml`. minSdk 26 → adaptive-only, legacy-PNG не нужны. В манифесте
  `android:icon`/`android:roundIcon`.
- **Подпись** (`app/build.gradle.kts`): `signingConfigs.release` читает секреты из **gitignored**
  `keystore.properties` (шаблон – коммитнутый `keystore.properties.example`). Ключ
  `remindy-release.jks` (RSA 2048, срок 10000 дн), alias `remindy`, DN `CN=Gde Lezhit, O=marlendd,
  C=RU`, отпечаток SHA-256 `09:43:56:5A:E1:78:C9:6F...`. **`.jks` и `keystore.properties` в
  `.gitignore`** (проверено `git check-ignore`). Если секретов нет – release собирается
  неподписанным, а не падает.
  - ⚠️ **КРИТИЧНО:** `remindy-release.jks` + пароль забэкапить ВНЕ репозитория. Потеря = нельзя
    выпускать обновления с тем же `applicationId`. Пароль сейчас только в `keystore.properties`
    на машине разработки.
- **release-buildType**: `isMinifyEnabled = false` (R8 выключен – риск для рефлексии
  Vosk/JNA/SQLCipher/Snowball; APK и так < лимита). `proguard-rules.pro` дополнен keep'ами
  Vosk/JNA/Room (Snowball/net.zetetic уже были) и подключён – готов на случай включения R8.
  `versionName` `0.1.0` → **`1.0.0`** (versionCode 1).
- **Статическая проверка** `app/build/outputs/apk/release/app-release.apk`: подписан релиз-ключом
  (apksigner v2 scheme ✓), `versionName 1.0.0`, label «Где лежит», иконка подхватилась
  (`res/BW.xml` на всех плотностях), разрешения RECORD_AUDIO+USE_BIOMETRIC, **INTERNET нет**,
  ABI только arm64-v8a, **65 МБ** < 80. lintVitalRelease зелёный.
- **Device-verified на Samsung A52 (Android 14):** удалил debug → поставил подписанный release →
  чистый старт показал онбординг → «Понятно» → главный экран (микрофон-подсказка) → системный
  запрос микрофона всплыл ПОСЛЕ онбординга (как задумано); крашей нет, INTERNET нет, versionName
  1.0.0, apkSigningVersion=2, label «Где лежит» в системном диалоге. Иконка-пин отрисована верно
  (терракота на песке, отверстие видно) – сверено рендером вектора и badging (`res/BW.xml`).
  Замечание: `run-as` на release не работает (не debuggable), поэтому проверку «файл БД не
  plaintext» на release не гоняли – код шифрования не менялся, в debug device-verified.

Пересобрать релиз: `./gradlew :app:assembleRelease` (нужен `keystore.properties` в корне).
...
3. **Release-buildType** – **сделан** (см. «Релиз v1.0.0» выше). Осталось – device-тест релиза
   и, при желании, включение R8 (keep-правила готовы).
4. **На будущее (по privacy.md)** – локальный зашифрованный экспорт базы под отдельным
   паролем – **сделан** (см. «Зашифрованный бэкап базы»).
