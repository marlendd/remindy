...
Этап 4 (Compose-UI + UX) – **код готов, собирается, JVM-тесты и lint зелёные, прошло
adversarial-ревью; device-verify на телефоне предстоит** (чек-лист ниже, раздел «Фаза 4»).
...
- **Undo при свайп-удалении** (`list/ListActivity`): свайп прячет строку оптимистично
  (`pendingItems`), snackbar «Удалено / Отмена»; реальное удаление – по истечении snackbar на
  `appScope` (переживает уничтожение Activity → уход с экрана в окне undo не теряет удаление,
  `onDestroy` доводит его; Room `@Delete` идемпотентен). Отмена возвращает строку; прунинг в
  `observeAll()` снимает строку без мигания «обратно».
...
- Список: свайп-влево → строка исчезает + snackbar «Удалено/Отмена». **Отмена возвращает запись.**
  Без отмены – запись реально удалена (переоткрыть список). Уйти Back в окне undo → запись всё равно
  удалена (ровно один раз). Тап по строке → правка: заголовок «Изменить запись», кнопка «Удалить»
  с подтверждением.
